    ...
    match receiver.get() {
        Value::Tuple(tuple) => match method {
            "length" => {
                ...
                Ok(Value::Number(tuple.len() as f64))
            }
            "to_list" => {
                if !args.is_empty() {
                    return Err(RuntimeError::ArityMismatch {
...
mod tests {
    ...
    fn test_tuple_length() {
        let tuple = Value::Tuple(vec![Value::Number(1.0), Value::String("test".to_string())]);
    ...
    fn test_tuple_length_single() {
        let tuple = Value::Tuple(vec![Value::Number(42.0)]);
        let receiver = ValueRef::Immutable(&tuple);
        let result = call_tuple_method(receiver, "length", vec![]).unwrap();

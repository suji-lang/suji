...
pub fn eval_assignment(
    ...
    eval_complex_assignment(target, rhs_value, env, registry)
}

/// Evaluate complex assignment with support for nested structures
pub fn eval_complex_assignment(
    target: &Expr,
    value: Value,
    env: Rc<Env>,
    registry: Option<&ModuleRegistry>,
    ...
    match target {
        ...
        Expr::Destructure { elements, .. } => match &value {
            Value::Tuple(items) | Value::List(items) => {
                ...
                // Return the original value (Tuple or List)
                Ok(value)
            }
            _ => Err(RuntimeError::DestructureInvalidTarget {
                message: "Can only destructure tuples and lists".to_string(),
            }),
        },
...
pub fn eval_deep_index_assignment(
    ...
    let index_value = eval_expr(index, env.clone(), registry)?;
...
fn update_nested_structure_in_env(
    target: &Expr,
    new_value: Value,
    env: Rc<Env>,
    registry: Option<&ModuleRegistry>,
    ...
    match target {
        ...
        Expr::Index {
            ...
            update_nested_structure_in_env(nested_target, updated_nested, env, registry)
        }
        Expr::MapAccessByName {
            target: nested_target,
            key: nested_key,
...
pub fn eval_compound_assignment(
    ...
    // Evaluate the target to get current value
    let current_value = eval_expr(target, env.clone(), registry)?;
    ...
    let binary_op = match op {
        ...
        CompoundOp::MultiplyAssign => BinaryOp::Multiply,
        CompoundOp::DivideAssign => BinaryOp::Divide,
        CompoundOp::ModuloAssign => BinaryOp::Modulo,
    };

    let new_value = super::expressions::eval_binary_op(&binary_op, current_value, rhs_value)?;

    // Assign the new value back
    eval_complex_assignment(target, new_value, env, registry)
}
...
mod tests {
    ...
    fn test_variable_assignment() {
        ...
        let result = eval_assignment(&target, &value, env.clone(), None).unwrap();
    ...
    fn test_no_variable_shadowing() {
        ...
        let result = eval_assignment(&target, &value, child_env.clone(), None).unwrap();
    ...
    fn test_complex_assignments() {
        ...
        let result2 = eval_assignment(&target2, &value2, env.clone(), None).unwrap();
        ...
        let result3 = eval_assignment(&target3, &value3, env.clone(), None).unwrap();

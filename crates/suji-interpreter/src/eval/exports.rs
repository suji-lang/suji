...
pub fn eval_export_body(
    ...
    match body {
        ExportBody::Map(spec) => {
            ...
            let result = eval_export(spec, env, registry)?;
            Ok(result.module)
        }
        ExportBody::Expr(expr) => {
            let value = eval_expr(expr, env, registry)?;
            Ok(value)
        }
    }
}
...
mod tests {
    ...
    fn test_eval_export_body_expr_literal_number() {
        ...
        let value =
            eval_export_body(&body, env, None).expect("export body evaluation should succeed");
    ...
    fn test_eval_export_body_expr_undefined_identifier_errors() {
        ...
        let err = eval_export_body(&body, env, None).unwrap_err();
    ...
    fn test_eval_export_body_map_returns_map() {
        ...
        let value = eval_export_body(&body, env, None).expect("map export should succeed");
        match value {
            Value::Map(map) => {
                assert_eq!(

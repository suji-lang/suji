...
pub fn eval_postfix_increment(
    target: &Expr,
    env: Rc<Env>,
    _registry: Option<&ModuleRegistry>,
) -> EvalResult<Value> {
    match target {
...
mod tests {
    ...
    fn test_postfix_increment() {
        ...
        let result = eval_postfix_increment(&expr, env.clone(), None).unwrap();
    ...
    fn test_postfix_decrement() {
        ...
        let result = eval_postfix_decrement(&expr, env.clone(), None).unwrap();
    ...
    fn test_increment_non_number_error() {
        ...
        let result = eval_postfix_increment(&expr, env, None);

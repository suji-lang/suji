...
use std::rc::Rc;
use suji_runtime::methods::{ValueRef, call_method};
...
pub fn builtin_print(args: &[Value]) -> Result<Value, RuntimeError> {
    ...
    let out_value = if args.len() == 2 {
        match &args[1] {
            Value::Nil => Value::Stream(Rc::new(suji_runtime::value::StreamHandle::new_stdout())),
        ...
        // Default to stdout
        Value::Stream(Rc::new(suji_runtime::value::StreamHandle::new_stdout()))
    };
...
mod tests {
    ...
    fn test_print_with_custom_stream() {
        ...
        let out = Value::Stream(Rc::new(
            suji_runtime::value::StreamHandle::new_memory_writable(),
        ));
        ...
        if let Value::Stream(handle) = out {
            let output = handle.take_memory_output().unwrap();
            assert_eq!(String::from_utf8(output).unwrap(), "abc".to_string());
        } else {

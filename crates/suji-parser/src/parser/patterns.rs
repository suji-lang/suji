...
impl Parser {
    ...
    pub(super) fn parse_pattern(&mut self) -> ParseResult<suji_ast::ast::Pattern> {
        ...
        if self.match_token(Token::LeftParen) {
            ...
            return Ok(suji_ast::ast::Pattern::Tuple { patterns, span });
        }
        ...
        if let Token::Number(n) = &self.peek().token {
            ...
            return Ok(suji_ast::ast::Pattern::Literal {
                value: suji_ast::ast::ValueLike::Number(n),
                span,
            });
        }
        ...
        if self.match_token(Token::True) {
            let span = self.previous().span.clone();
            return Ok(suji_ast::ast::Pattern::Literal {
                value: suji_ast::ast::ValueLike::Boolean(true),
                span,
            });
        }
        ...
        if self.match_token(Token::False) {
            let span = self.previous().span.clone();
            return Ok(suji_ast::ast::Pattern::Literal {
                value: suji_ast::ast::ValueLike::Boolean(false),
                span,
            });
        }
        ...
        if self.match_token(Token::Nil) {
            let span = self.previous().span.clone();
            return Ok(suji_ast::ast::Pattern::Literal {
                value: suji_ast::ast::ValueLike::Nil,
                span,
            });
        }
        ...
        if self.match_token(Token::StringStart) {
            ...
            while !self.check(Token::StringEnd) && !self.is_at_end() {
                if let Token::StringText(text) = &self.peek().token {
                    let text = text.clone();
                    self.advance();
                    parts.push(suji_ast::ast::StringPart::Text(text));
            ...
            if parts.len() == 1
                && let suji_ast::ast::StringPart::Text(text) = &parts[0]
            {
                return Ok(suji_ast::ast::Pattern::Literal {
                    value: suji_ast::ast::ValueLike::String(text.clone()),
                    span,
                });
            }
        ...
        if let Token::Identifier(name) = &self.peek().token {
            ...
            let span = self.advance().span.clone();
            return Ok(suji_ast::ast::Pattern::Literal {
                value: suji_ast::ast::ValueLike::String(name),
                span,
            });
        }
